...
var Share = function (data, userAgent) {
    ...
    this.screen = data.screen.map(datum => (typeof datum == 'number' ? datum : null));

    this.samples = collectedTests.map(test => {
        var samples = data.samples[test],
            avg = average(samples),
            max = getMax(samples),
            min = getMin(samples),
            stddev = getStandardDeviation(samples); 
